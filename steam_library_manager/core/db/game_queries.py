...
class GameQueryMixin:
    """Mixin providing single-game CRUD. Requires ``self.conn``."""

    def insert_game(self, entry: DatabaseEntry) -> None:
        """INSERT OR REPLACE a game. Does not commit."""
        now = int(time.time())
    ...
    def update_game(self, entry: DatabaseEntry) -> None:
        """Update a game, preserving created_at and protecting real names from placeholders."""
        now = int(time.time())
    ...
    def get_app_type_lookup(self) -> dict[str, tuple[str, str]]:
        """Fast lookup: app_id -> (app_type, name) for all games."""
