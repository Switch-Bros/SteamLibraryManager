...
from PyQt6.QtCore import QFileSystemWatcher, QThread, QTimer
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # appinfo.vdf external-change watcher (initialized post-bootstrap)
        self._appinfo_watcher: QFileSystemWatcher | None = None
        self._appinfo_reload_timer: QTimer | None = None
    ...
    def _on_bootstrap_complete(self):
        ...
        # Watch appinfo.vdf for external overwrites (Steam refreshes,
        # CDN-driven updates, etc.) so we can re-apply the user's
        # modifications immediately when Steam wipes them out.
        self._setup_appinfo_watcher()

    def _setup_appinfo_watcher(self):
        # Initialize a QFileSystemWatcher on the appinfo.vdf path.
        # Re-applies modifications whenever Steam rewrites the file.
        if getattr(self, "_appinfo_watcher", None) is not None:
            return  # already set up

        if not self.appinfo_manager or not self.appinfo_manager.appinfo_path:
            return

        appinfo_path = self.appinfo_manager.appinfo_path
        if not appinfo_path.exists():
            return

        try:
            self._appinfo_watcher = QFileSystemWatcher(self)
            self._appinfo_watcher.addPath(str(appinfo_path))
            self._appinfo_watcher.fileChanged.connect(self._on_appinfo_file_changed)

            # Debounce: Steam often writes the file in several short bursts.
            # Coalesce them so we only reload once per logical overwrite.
            self._appinfo_reload_timer = QTimer(self)
            self._appinfo_reload_timer.setSingleShot(True)
            self._appinfo_reload_timer.setInterval(800)
            self._appinfo_reload_timer.timeout.connect(self._reload_appinfo_after_external_change)

            import logging

            logger = logging.getLogger("steamlibmgr.main_window")
            logger.info(t("logs.appinfo.watcher_started"))
        except Exception as e:
            import logging

            logger = logging.getLogger("steamlibmgr.main_window")
            logger.warning(t("logs.appinfo.watcher_failed", error=str(e)))
            self._appinfo_watcher = None

    def _on_appinfo_file_changed(self, _path: str):
        # Slot for QFileSystemWatcher.fileChanged. Ignores our own writes
        # and otherwise schedules a debounced reload.
        if not self.appinfo_manager:
            return

        # Ignore the change event we just generated by writing to the file
        # ourselves. The bool flag is only reliable while the synchronous
        # write call is on the stack; inotify events arrive async, often a
        # second later, so we also check a short timestamp window.
        if getattr(self.appinfo_manager, "_writing_to_vdf", False):
            return

        import time

        last_self_write = getattr(self.appinfo_manager, "_last_self_write_ts", 0.0)
        if last_self_write and (time.time() - last_self_write) < 2.0:
            return

        # Some editors (and Steam) replace the file via rename, which makes
        # QFileSystemWatcher drop the watch. Re-add it on the next tick.
        watcher = self._appinfo_watcher
        if watcher is not None and self.appinfo_manager.appinfo_path:
            path_str = str(self.appinfo_manager.appinfo_path)
            if path_str not in watcher.files():
                QTimer.singleShot(100, lambda: watcher.addPath(path_str))

        if self._appinfo_reload_timer is not None:
            self._appinfo_reload_timer.start()

    def _reload_appinfo_after_external_change(self):
        # Reload appinfo.vdf from disk and re-apply modifications.
        # Triggered by the debounced file-watcher timer.
        if not self.appinfo_manager:
            return

        # Skip if we ended up writing the file ourselves between scheduling
        # and now.
        if getattr(self.appinfo_manager, "_writing_to_vdf", False):
            return
        ...
        logger = logging.getLogger("steamlibmgr.main_window")
        logger.info(t("logs.appinfo.external_change"))

        # load_appinfo() now triggers verify_and_reapply() automatically,
        # so this single call is enough to detect drift and re-write.
        try:
