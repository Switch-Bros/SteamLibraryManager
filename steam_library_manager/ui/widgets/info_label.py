...
def build_detail_grid(
    ...
    """Build a one-row QGridLayout with a bold title and InfoLabels."""
    container = QWidget()
...
def set_info_label_value(label: QLabel, value: str, color: str = "") -> None:
    """Update the bold value portion of an InfoLabel."""
...
def update_hltb_label(label: QLabel, hours: float, dash: str) -> None:
    """Update an HLTB InfoLabel with formatted hours or a dash."""
    parts = label.text().split(":</span>")
    prefix = parts[0] + ":</span>" if len(parts) > 1 else ""
...
def _format_rating_html(
    ...
    """Return a styled HTML string for a rating label."""
